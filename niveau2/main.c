 ...
 */
void touche_arret(ball_t *b1, ball_t *b2){
  if (touche(b1,b2)==1){
    double a=b1->x;
    double b=b1->y;
    double c=b2->x;
...
/**
 * \brief La fonction quand la boule blanche rencontre la boule de couleur, elle s’arrˆete.
 */
...
if(ball->x+BALL_SIZE/2<609||ball->x+BALL_SIZE/2>679){ 
...
int main( int argc, char* args[] )
{
    ...
    while(!is_game_over(&world)){
        ...
        update_data(&world);
        set_speed(&world.ball,0,0);
	set_position(&world.ball,0,0);
	touche_arret(&world.ball,&world.balla);
	dispa(&world);
